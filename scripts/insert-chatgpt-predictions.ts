/**
 * Creates ChatGPT's user account and inserts its bracket + match predictions.
 *
 * Usage:
 *   npx tsx scripts/insert-chatgpt-predictions.ts [LEAGUE_CODE]
 *
...
const CHATGPT_NAME  = "ChatGPT IA 🤖";
const PREDICTIONS_FILE = path.join(__dirname, "chatgpt-predictions.json");
...
async function main() {
  ...
  console.log("👤 Creating ChatGPT auth user...");
  ...
  if (existing) {
    chatgptUserId = existing.id;
    console.log(`   Already exists: ${chatgptUserId}`);
  } else {
    const { data: created, error: createErr } = await supabase.auth.admin.createUser({
      email: CHATGPT_EMAIL,
      email_confirm: true,
      user_metadata: { full_name: CHATGPT_NAME },
    });
    if (createErr || !created?.user) die("Failed to create ChatGPT auth user", createErr);
    chatgptUserId = created.user.id;
    console.log(`   Created: ${chatgptUserId}`);

    await supabase
      .from("users")
      .update({ name: CHATGPT_NAME })
      .eq("id", chatgptUserId);
  }
  ...
  console.log("\n🤖 ChatGPT IA está listo en la quiniela!");
  console.log(`   user_id: ${chatgptUserId}`);
