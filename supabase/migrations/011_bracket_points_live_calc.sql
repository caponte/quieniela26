-- Update calculate_bracket_points_all to use current standings (same logic as
-- the live bracket viewer) instead of requiring all 6 group matches finished.
-- The trigger already fires on every match update, so bracket_points stays
-- in sync with the live viewer after each result.
...
BEGIN
  ...
  FOR v_bp IN SELECT * FROM public.bracket_predictions LOOP
    ...
    -- ── Group stage ──────────────────────────────────────────────────────────
    -- Score based on current standings (same as live bracket viewer).
    -- A group with no finished matches simply returns no standings → no pts.
...
-- Backfill: recalculate now with current standings.
